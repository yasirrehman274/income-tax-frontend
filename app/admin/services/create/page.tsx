...
export default function CreateServicePage() {
    ...
    const checkSlug = async () => {
      ...
      try {
        const res = await fetch(
          `/api/services/check-slug/${form.slug}`,
          { credentials: "include" }
        );
  ...
  const validateForm = () => {
    ...
    if (
      !validate(form.long_description, "required").valid ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const featureImageUrl = getCurrentFile("featureImage")
        ? await uploadFile(getCurrentFile("featureImage")!, "services")
        : null;
              ...
              <div className="border border-gray-300 dark:border-gray-600 p-4 rounded-2xl space-y-6">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="title">
                      Service Title{" "}
                      <StatusIcon value={form.title} type="required" />
                    </Label>
                    <Input
                      id="title"
                      value={form.title}
                ...
                <div className="space-y-2">
                  ...
                  <TiptapEditor
                    ...
                    onChange={(val) =>
                      setForm({ ...form, long_description: val })
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label>
                    Featured Image{" "}
                    <StatusIcon value={form.featureImage} type="required" />
                  </Label>
                  <ImageDropZone
                    value={form.featureImage}
                    onChange={(val) =>
                      setForm((prev) => ({ ...prev, featureImage: val }))
                    }
                    alt={form.featureImageAlt}
                    onAltChange={(val) =>
                      setForm((prev) => ({ ...prev, featureImageAlt: val }))
                    }
